...
public:
    static const bool value = sizeof(test<T>(0)) == siezof(char);
};
...
// 是迭代器类型，判断是否能隐式转换为input_iterator_tag或output_iterator_tag
template<class Iterator>
struct iterator_traits_helper<Iterator, true>
    : public iterator_traits_impl<iterator, std::is_convertible<typename Iterator::iterator_category, input_iterator__tag>::value || std::is_convertible<typename Iterator::iterator_category, output_iterator__tag>::value> {};
...
// 萃取迭代器的 value_type，就是用了个类型转换，从而获得类型
template<class Iterator>
...
{
    // iterator_category(first)进行隐式实例化
...
// advance 的 random_access_iterator_tag 版本
